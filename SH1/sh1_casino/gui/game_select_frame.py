# -*- coding: utf-8 -*-
"""gui/game_select_frame.py - 게임 선택 화면"""
...
class GameSelectFrame(tk.Frame):
    def __init__(self, parent, app):
        ...
        tk.Label(self, text="게임 선택", font=styles.FONT_TITLE,
        ...
        games = [
            ("🂡", "블랙잭", "규칙 기반 딜러와의 1:1 대결\n21에 가장 가깝게!", "BlackjackFrame"),
            ("♣", "포커 (홀덤)", "규칙 기반 AI와의 헤즈업 포커\n최고의 5장 조합으로 승부", "PokerFrame"),
            ("🏇", "사다리타기", "경마 컨셉의 확률 게임\n레인을 골라 배당을 노려라", "LadderFrame"),
        ]

        for i, (icon, title, desc, frame_name) in enumerate(games):
            card = tk.Frame(cards_frame, bg=styles.BG_PANEL, padx=26, pady=22,
                             highlightbackground=styles.GOLD, highlightthickness=1)
            ...
            btn = tk.Button(card, text="플레이", width=14,
                             command=lambda f=frame_name: self.app.show_frame(f))
            styles.style_button(btn)
            btn.pack()
